...
export const RedeemModalSimple: React.FC<RedeemModalProps> = ({
  visible,
  onClose,
  availableBalance,
  onSuccess,
  ...
  if (!visible) {
    ...
    if (redeemAmount || error || isLoading || isSuccess) {
      setRedeemAmount('');
      setError('');
      setIsLoading(false);
      setIsSuccess(false);
  ...
  const handleSubmit = async () => {
    const validationError = validateAmount(redeemAmount);
    if (validationError) {
  ...
  const isValid = !validateAmount(redeemAmount) && redeemAmount.trim() !== '' && accountNumber.length >= 9 && accountNumber.length <= 18;
